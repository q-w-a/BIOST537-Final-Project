...
# get kaplan-meier curve by a continuous variable 
# by splitting the continuous variable into two groups
# by whether the value of the variable was above or below 
# the median 
get_continuous_km <- function(var, nice_name) {
  
  # calculate median of the continuous variable of interest 
  median_var <- bmt[[var]] %>% median(na.rm=TRUE)
  
  bmt_w_thresh <- bmt %>% mutate(
    !!sym(var) := ifelse(!!sym(var) > median_var,
                          paste0("> ", median_var),
                          paste0("≤ ", median_var)))
  
  # Fit model with thresholded version of continuous covariate
  fit <- survfit(as.formula(paste0("Surv(tdfs, deltadfs) ~ ", var)),
     data = bmt_w_thresh)
  ...
  ggsurvfit(fit)$data %>%
    ...
    viridis::scale_color_viridis(discrete=TRUE,
                                 option="mako", end=.8, begin=.2) +
    viridis::scale_fill_viridis(discrete=TRUE,
                                 option="mako", end=.8, begin=.2) +
    labs(y="Disease-Free Survival", x= "Time (Days)",
          title=paste0("By ", nice_name),
         fill="", color="") +
...
# get kaplan meier curve by a discrete variable 
get_discrete_km <- function(var, nice_name) {
  
  # Fit model with discrete covariate
  fit <- survfit(as.formula(paste0("Surv(tdfs, deltadfs) ~ ", var)),
     data = bmt)
...
get_continuous_survival_curve <- function(var, nice_name) {
  
  mod <- coxph(as.formula(paste0("Surv(tdfs, deltadfs) ~ ", var)),
     data = bmt)
  
  newdat <- tibble(
    !!var := quantile(bmt[[var]],probs= c(.25,.75), na.rm=TRUE))
...
# get kaplan meier curve corresponding to given covariate 
get_km_curve <- function(var, nice_name) {
  if(is.factor(bmt[[var]])) {
    plot <- get_discrete_km(var, nice_name)
  }
  else {
    plot <- get_continuous_km(var, nice_name)
  }
  return(plot)
}


# iterate over all baseline variables of interest 
pltlist <- map2(covariates,nice_names, get_km_curve)
...
(pltlist[[1]] + pltlist[[2]])/ (pltlist[[3]] + pltlist[[4]]) / (pltlist[[5]] + pltlist[[6]]) / (pltlist[[7]] + pltlist[[8]])/(pltlist[[9]] + pltlist[[10]]) + plot_annotation(
  title = "Kaplan-Meier Curves by Baseline Variables of Interest",
  theme=theme_c(plot.title=element_text(face="bold",hjust=.5, size=20)))

# save plot 
ggsave(here("figures/km_by_baseline.jpeg"))
...
To assess the association between prophylactic use of methotrexate and the risk of developing aGVHD, we considered a fully adjusted Cox proportional hazards model, adjusting for the confounders age, fab classification, wait time until transplant, donor age, donor sex, cmv, sex, donor cmv, and disease group. PLACEHOLDER - JUSTIFY CHOICE OF CONFOUNDERS SOMEWHERE. Because hospital is perfectly collinear with the prophylactic use of methotrexate, we did not adjust for hospital. The time-to-event variable of interest was time from transplantation (time at enrollment) to time of the develoment of aGVHD. 

While death is a competing event for the development of aGVHD, this is not a problem in our sample because no patient died before developing aGVHD, and hence we do not need to explicitly handle competing events for this analysis.
...
bmt <- bmt %>%
  mutate(sex_mismatch = paste0(donormale, "->", male),
        ...
        #  sex_mismatch = ifelse(sex_mismatch == "Female->Male", 1, 0),
        # cmv_mismatch = ifelse(cmv_mismatch ==
        #                         "CMV Negative->CMV Positive",
        #                       1,0)
        )
...
To assess whether the recovery of normal platelet levels associated with disease-free survival, we fit a Cox proportional hazards model with the recovery of platelet levels as a time-varying covariate, adjusting for the confounders age, FAB classification, wait time until transplant, donor age, donor-recipient sex mismatch, donor-recipient CMV mismatch, and disease group. 

While death could act as a competing event, no patient in our sample died before their return to normal platelet levels, so it was not necessary to handle competing events for this analysis.

To evaluate whether there is association between the recovery of normal platelet levels and the risk of relapse, we do need to handle the fact that death is a competing event. To do so, we use a semiparametric proportional sub-distribution odds model 
...
dat <- bmt %>% 
  select(age,fab, waittime, disgroup, 
         deltap, tp, deltadfs, deltar, deltas, sex_mismatch,cmv_mismatch,donorage,
         tdfs) %>%
  mutate(id=row_number())
...
dat <- bmt %>% 
  select(age,fab, waittime, cmv_mismatch, sex_mismatch, male, disgroup, 
         deltap, tp, deltadfs, donorage,donormale,
         tdfs) %>%
  mutate(id=row_number())
...
model <- coxph( Surv(tstart, tstop, endpt) ~ age + fab + waittime + 
         cmv + male + disgroup + plat +donorage+donormale, data=dat) 
...
get_marginal_surv <- function(dat) {
  ...
  surv_treat %>%
    left_join(surv_no_treat)
  
}

boot_surv <- map_df(1:100, ~ {
  ind <- sample(1:nrow(bmt), replace=TRUE)
  boot_samp <- bmt[ind,]
  get_marginal_surv(boot_samp)
}) 
...
boot_summary <- boot_surv |>
  group_by(time) |>
  summarise(
    S1_lo = quantile(surv_treat, 0.025, na.rm = TRUE),
    S1_hi = quantile(surv_treat, 0.975, na.rm = TRUE),
    S0_lo = quantile(surv_no_treat, 0.025, na.rm = TRUE),
    S0_hi = quantile(surv_no_treat, 0.975, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(-c(time)) %>% 
  mutate(treatment = ifelse(grepl("S1", name),
                             "Prophylactic Use of Methotrexate",
                            "No Prophylactic Use of Methotrexate"),
         new_name = ifelse(grepl("lo", name), "lower","upper")) %>%
  select(-name) %>%
  pivot_wider(values_from=value,names_from=new_name)


boot_surv %>%
  mutate(difference=surv_treat-surv_no_treat) %>%
  group_by(time) %>%
  summarize(lower=quantile(difference,.025),
            upper=quantile(difference,.975)) %>%
  ggplot(aes(x=time, ymin =lower, ymax=upper)) +
  geom_errorbar()

surv_est_dir6 %>%
  left_join(boot_summary) %>%
  mutate(maxtime=lead(time,n=1)) %>%
  ggplot(aes(x=time, y=mean_surv, color=treatment))+
  geom_step() +
  coord_cartesian(xlim=c(0,150)) +
  theme_c() 
...
+
  viridis::scale_color_viridis(
      discrete=TRUE, option="mako", end=.8, begin=.2) +
  labs(x="Time (Days)", y = "Event-Free Survival") +
  geom_rect(aes(xmin = time, xmax = maxtime, 
...
model <- coxph(Surv(ta, deltaa) ~mtx + age + donorage +
                 waittime + fab + 
                 donormale + male +
                 cmv + donorcmv +
                 disgroup, data=bmt
